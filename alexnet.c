...
int main(void) {
    mt_tensor *img = mt_tensor_load_image("cat.jpg");
    ...
    // Flatten
    mt_tensor_reshape_inplace(h5_pool, MT_ARR_INT(1, h5_pool->shape[0] * 36),
                              2);
    mt_tensor_debug_info(h5_pool);
    ...
    for (int i = 0; i < mt_tensor_count_element(fc3); ++i) {
